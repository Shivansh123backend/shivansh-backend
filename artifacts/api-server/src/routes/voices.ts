...
// ── POST /voices/:id/sample — stream a short TTS preview (any provider) ───────
// Supports elevenlabs, cartesia, and deepgram voices. Uses generateTTSFromProvider
// so the same provider the voice is configured with is used for the preview.
router.post("/voices/:id/sample", authenticate, async (req, res): Promise<void> => {
  ...
  try {
    // generateTTSFromProvider stores the audio in the in-memory cache and returns
    // a URL like /api/audio/:token — we extract the token and serve the bytes directly
    // so the browser gets audio/mpeg without needing a second round-trip.
    const audioUrl = await generateTTSFromProvider(provider, voice.voiceId, sampleText);
    const token = audioUrl.split("/").pop()!;
    const cached = audioCache.get(token);

    if (!cached) {
      logger.error({ voiceId: voice.voiceId, provider, audioUrl }, "TTS sample: cache miss after generation");
      res.status(502).json({ error: "TTS generation failed — cache miss" });
      return;
    }
    ...
    audioCache.delete(token); // immediately free memory — already served
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
